...
class CacheManager {
  ...
  constructor(client: Client, opts: CacheOptions) {
    ...
    if (!opts.$options || !opts.$options.use_db) {
      this._channels = (!opts.channels?.enabled)
        ? null
        : new CacheMap(null, opts.channels.max);
      this._guilds = (!opts.guilds?.enabled)
        ? null
        : new CacheMap(null, opts.guilds.max);
      this._users = (!opts.users?.enabled)
        ? null
        : new CacheMap(null, opts.users.max);
      this._invites = (!opts.invites?.enabled)
        ? null
        : new CacheMap(null, opts.invites.max);
      this._messages = (!opts.messages?.enabled)
        ? null
        : new CacheMap(null, opts.messages.max);
    } else {
  ...
  public add(type: Channel | User | Message | Guild): void {
    const typeCache: string =
      type.constructor.name.toLowerCase().replace(/(text|voice)/ig, "") + "s";
