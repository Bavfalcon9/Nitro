...
class EventHandler {
  ...
  public handleEvent(pk: EventPacket): void {
    ...
    const funcName = "on" +
      name.toLowerCase().split("_").map((e) =>
  ...
  public onMessageCreate(data: any): void {
    const channel = this.client.cacheManager.channels.get(data.channel_id);
    const guild = this.client.cacheManager.guilds.get(data.guild_id);
    let message: Message;
    ...
    this.client.cacheManager.add(message);
  ...
  public onChannelCreate(data: any): void {
    ...
    switch (type) {
      case "text":
        ...
        this.client.cacheManager.add(channel);
        break;
      case "voice":
  ...
  public onGuildCreate(data: any): void {
    ...
    this.client.cacheManager.add(guild);
    // todo: members.
